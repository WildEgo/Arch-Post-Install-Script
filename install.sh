...
makepkg -si
cd ..
sudo yay -S --noconfirm nvidia xorg-server egl-wayland wget git lightdm lightdm-gtk-greeter ttf-google-fonts-git kde-desktop kscreen plasma-wayland-session neofetch exa alsa-utils kitty-git fish nginx-mainline starship-git nvm php-fpm python mariadb docker code filezilla android-studio google-chrome firefox opera lutris legendary steam vlc spotify ufw obs-studio-git virtualbox ffmpeg ktorrent timeshift-bin kdeconnect htop beekeeper-studio-bin go
